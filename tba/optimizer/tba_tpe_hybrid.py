...
Phase 2 (remaining 60%): Optuna's real TPE takes over, warm-started with
    ALL of TBA's Phase 1 history injected via study.add_trial().

Each phase does what it's best at:
    - TBA handles dangerous early exploration in crash-heavy spaces
    - Optuna's TPE handles smart model-guided optimization
"""
...
class TBATPEHybrid(BaseOptimizer):
    ...
    Phase 1: Feasible-first SA with adaptive temperature (first 40% of budget).
    Phase 2: Optuna TPE warm-started from Phase 1 history (remaining 60%).
    """
    ...
    def _tba_ask(self) -> dict[str, Any]:
        ...
        # Initialize from history after random phase
        if self.trial_count == self.n_initial_random:
    ...
    def _inject_trial(self, config: dict[str, Any], result: EvalResult) -> None:
        """Inject a single trial from TBA history into the Optuna study."""
        # Build params dict — only include variables that were active
        params = {}
        active_dists = {}
        for name in self.search_space._all_names:
            if name in config:
    ...
    def _sample_from_trial(self, trial: optuna.trial.Trial) -> dict[str, Any]:
        """Sample using Optuna's conditional suggest_* — gives TPE full tree structure."""
        config: dict[str, Any] = {}
