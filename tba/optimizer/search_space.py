...
class SearchSpace:
    ...
    def sample_random(self, rng: random.Random | None = None) -> dict[str, Any]:
        ...
        Strategy: sample unconditional variables first, then conditionals
        whose parents are already set. Repeat until all activatable
        variables are filled in.
        """
        rng = rng or random.Random()
        config: dict[str, Any] = {}

        # Iterative: keep resolving until stable
    ...
    def propose_neighbor(
        ...
        if structural and (not numeric or rng.random() < p_structural):
            ...
            # Mutate a numeric variable with temp-proportional step
    ...
    def config_distance(self, a: dict[str, Any], b: dict[str, Any]) -> float:
        """Normalized distance between two configs, handling mixed types and
        conditional structure. Inactive variables are treated as matching
        (no penalty for variables that don't exist in one config)."""
        ...
        for name in all_names:
            ...
            if va is None or vb is None:
                # One config doesn't have this variable — maximum distance for this dim
                total += 1.0
                count += 1
                continue
    ...
    def is_valid(self, config: dict[str, Any]) -> bool:
        ...
        and no inactive variables are set."""
        active = set(self.active_variables(config))
        # All active vars must be present
        for name in active:
    ...
    def _eval_condition(self, condition: str, config: dict[str, Any]) -> bool:
        """Safely evaluate a condition string against current config values."""
        try:
    ...
    def _sample_variable(self, v: VariableDef, rng: random.Random) -> Any:
        if v.var_type == "categorical":
            ...
            if v.log_scale:
                ...
                # Snap to nearest power of 2 within bounds
    ...
    def _perturb_numeric(
        ...
        """Perturb a numeric variable. Step size ~ temperature * range."""
        span = v.high - v.low
        sigma = temperature * span * 0.3  # 30% of range at T=1
    ...
    def _resolve_hierarchy(self, config: dict[str, Any], rng: random.Random) -> None:
        ...
        active = set(self.active_variables(config))
        # Drop variables that are no longer active
        to_drop = [k for k in list(config.keys()) if k not in active and k in self.variables]
        for k in to_drop:
            del config[k]
        # Sample newly active variables that are missing
        for _ in range(10):
            changed = False
            for name in self._all_names:
                if name in config:
