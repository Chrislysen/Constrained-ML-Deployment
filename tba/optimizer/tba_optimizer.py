"""TBA — Thermal Budget Annealing optimizer.

Feasible-first, crash-aware optimization for hierarchical mixed-variable
search spaces with hard constraints and tiny trial budgets.

Phase 1 (Feasibility): Adaptive SA to find a feasible config fast.
Phase 2 (Optimization): KDE-based TPE sampling within the feasible region,
    with RF surrogate as crash/constraint pre-filter. Annealing as fallback
    until enough feasible observations exist for the KDE.
"""
...
from tba.optimizer.feasible_tpe import FeasibleTPESampler
...
class TBAOptimizer(BaseOptimizer):
    """Thermal Budget Annealing optimizer.
    ...
    Key components:
      - Feasible-first phase transition
      - Adaptive temperature with reheating (phase 1 + fallback)
      - KDE-based TPE sampling within feasible region (phase 2)
      - OOB-gated RF surrogate as crash/constraint pre-filter
      - Adaptive structural mutation rate (0.5 -> 0.15)
      - Elite restart in first 70% of budget
      - Decaying snap-back probability
    """
    ...
    def __init__(
        ...
        # Adaptive structural mutation rate
        ...
        # Surrogate (RF for feasibility pre-filtering)
    ...
    def ask(self) -> dict[str, Any]:
        # Phase 0: Initial random exploration
        ...
        # Elite restart — only in first 70% of budget
        ...
        # Optimization phase: use feasible-TPE if enough data, else annealing
    ...
    def _feasible_tpe_ask(self, feasible_hist: list[tuple[dict, EvalResult]]) -> dict[str, Any]:
        ...
        if not self._ftpe._fitted:
            self._ftpe.fit(feasible_hist)
            if not self._ftpe._fitted:
                # Not enough data — fall back to annealing
    ...
    def tell(self, config: dict[str, Any], result: EvalResult) -> None:
        ...
        # During initial random phase, just collect data
        if self.trial_count <= self.n_initial_random:
            return
    ...
    # ------------------------------------------------------------------
    # Phase 1: Feasibility search (UNCHANGED)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Phase 2: Objective optimization
    # ------------------------------------------------------------------
    ...
    def _tell_optimization_phase(self, config, result, violation, obj):
        ...
        # Decaying snap-back to global best
